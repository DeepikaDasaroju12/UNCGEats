...
export class BackendService {
  ...
  createUser(user: User): Observable<any> {
    ...
    user.Id = -1
    return this.http.post<any>(new URL( 'user/createUser', 'http://localhost:8000' ).toString(), user);
  }
  ...
  createOwner(owner: Owner): Observable<any> {
    ...
    owner.Id = -1
    return this.http.post<any>(new URL( 'owner/createOwner', 'http://localhost:8000' ).toString(), owner);
  }

  createCanteen(canteen: Canteen): Observable<any> {
    canteen.CreatedTime = undefined
    canteen.LastLogged = undefined
    canteen.LastUpdated = undefined
    canteen.Id = -1
    return this.http.post<any>(new URL( 'canteen/createCanteen', 'http://localhost:8000' ).toString(), canteen);
  }
  ...
  createOrder(order: Order): Observable<any> {
    ...
    order.Id = -1
    return this.http.post<any>(new URL( 'canteen/createCanteen', 'http://localhost:8000' ).toString(), order);
  }
